...
impl RawInjector {
    ...
    pub fn new() -> Result<Self> {
        ...
        let raw_fd_v4 = sock_v4.into_raw_fd();
        ...
        unsafe {
            ...
            if ret < 0 {
                let err = std::io::Error::last_os_error();
                log::warn!(
                    "Failed to set IPv6 socket mark ({}): {}. \
                     Packet injection may still work, but eBPF filtering might process injected packets.",
                    err.raw_os_error().unwrap_or(-1),
                    err
                );
        ...
        let udp_injector =
            UdpFragmentInjector::new().context("Failed to create UDP fragment injector")?;
    ...
    pub fn inject_fake_packet(
        ...
        let tcp_checksum = calculate_tcp_checksum(src_ip, dst_ip, &packet[20..]);
        ...
        debug!(
            "Successfully injected {} bytes to {}:{}",
            sent, dst_ip, dst_port
        );
        Ok(())
    }

    /// Inject a fake TCP packet for IPv6 flow
    pub fn inject_fake_packet_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
        ...
        debug!(
            "Injecting fake IPv6 packet: [{}]:{} -> [{}]:{}, seq={}, ack={}, flags={:02x}",
            src_ip, src_port, dst_ip, dst_port, seq, ack, flags
        );
        ...
        if sent < 0 {
            let err = std::io::Error::last_os_error();
            return Err(anyhow!(
                "Failed to send raw IPv6 packet to [{}]:{} - {} (os error {}). \
                 Ensure the destination is reachable and you have network access.",
                dst_ip,
                dst_port,
                err,
    ...
    pub fn inject_split_packets(
        ...
        if split_pos == 0 || split_pos >= payload.len() {
            return (
                Err(anyhow!(
                    "Invalid split position: {} (payload len: {})",
                    split_pos,
                    payload.len()
                )),
    ...
    /// IPv6 version of TCP split packet injection.
    pub fn inject_split_packets_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
        ...
        let first_result = self.inject_fake_packet_v6(
            src_ip,
            dst_ip,
            src_port,
            dst_port,
        ...
        let second_seq = seq.wrapping_add(first_part.len() as u32);
        let second_flags = if flags & 0x01 != 0 { 0x18 | 0x01 } else { 0x18 };
        let second_result = self.inject_fake_packet_v6(
            src_ip,
            dst_ip,
            src_port,
            dst_port,
    ...
    pub fn inject_tls_split_packets(
        ...
        if payload.len() < TLS_RECORD_HEADER_LEN {
            return (
                Err(anyhow!(
                    "TLS payload too short: {} bytes (need at least 5 for header)",
                    payload.len()
                )),
        ...
        second_record
            .extend_from_slice(&payload[split_pos..TLS_RECORD_HEADER_LEN + original_length]);
    ...
    /// IPv6 version of TLS record split.
    pub fn inject_tls_split_packets_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
    ...
    pub fn inject_fake_with_offset(
        ...
        if fake_offset < 0 {
            ...
            self.inject_fake_packet(
                src_ip, dst_ip, src_port, dst_port, rst_seq, ack, 0x04, // RST flag
                None,
            )
        } else {
    ...
    /// IPv6 version of fake offset injection.
    pub fn inject_fake_with_offset_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
        ...
        if fake_offset < 0 {
            ...
            self.inject_fake_packet_v6(
                src_ip, dst_ip, src_port, dst_port, rst_seq, ack, 0x04, // RST
                None,
            )
        } else {
            let fake_seq = seq.wrapping_add(fake_offset as u32);
    ...
    pub fn inject_disorder_packets(
        ...
        if split_pos == 0 || split_pos >= payload.len() {
            return (
                Err(anyhow!(
                    "Invalid split position for disorder: {} (payload len: {})",
                    split_pos,
                    payload.len()
                )),
                Err(anyhow!("Invalid split position")),
        ...
        match &second_result {
            Ok(_) => info!(
                "[DISORDER] Second part sent first (out-of-order): {} bytes at seq={}",
                second_part.len(),
                second_seq
            ),
        ...
        match &first_result {
            Ok(_) => info!(
                "[DISORDER] First part sent second: {} bytes at seq={}",
                first_part.len(),
                seq
            ),
    ...
    /// IPv6 version of packet disorder injection.
    pub fn inject_disorder_packets_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
    ...
    /// IPv6 version of OOB injection.
    pub fn inject_oob_packet_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
        ...
        let flags = 0x38; // URG|PSH|ACK
        let mut tcp_packet = build_tcp_header_with_urgent(
            src_port,
            dst_port,
            seq,
            ack,
        ...
        if sent < 0 {
            let err = std::io::Error::last_os_error();
            return Err(anyhow!(
                "Failed to send OOB IPv6 packet to [{}]:{} - {} (os error {})",
                dst_ip,
                dst_port,
                err,
                err.raw_os_error().unwrap_or(-1)
    ...
    /// Get the raw IPv6 socket file descriptor
    ///
    /// # Safety
    /// The caller must ensure proper handling of the raw FD to avoid resource leaks
    #[allow(dead_code)]
...
impl UdpFragmentInjector {
    ...
    pub fn new() -> Result<Self> {
        ...
        let raw_fd_v4 = sock.into_raw_fd();
        ...
        if raw_fd_v6 < 0 {
            let err = std::io::Error::last_os_error();
            return Err(anyhow!(
                "Failed to create raw IPv6 socket for UDP fragmentation: {} (os error {})",
                err,
                err.raw_os_error().unwrap_or(-1)
            ));
        }
        ...
        if ret < 0 {
            let err = std::io::Error::last_os_error();
            return Err(anyhow!(
                "Failed to set IPV6_HDRINCL on raw socket: {} (os error {})",
                err,
                err.raw_os_error().unwrap_or(-1)
            ));
        }
        ...
        info!("UDP Fragment injector created for IPv4 and IPv6");
        Ok(Self {
            sock_v4: raw_fd_v4,
            sock_v6: raw_fd_v6,
            tuning: InjectorTuning::default(),
    ...
    pub fn inject_fragmented_udp(
        ...
        let mut frag_payload_size = usize::from(if frag_size == 0 { 8 } else { frag_size });
        if frag_payload_size < 8 {
            frag_payload_size = 8;
        }
        frag_payload_size = (frag_payload_size / 8) * 8;
        if frag_payload_size == 0 {
            frag_payload_size = 8;
        }

        info!(
            "Fragmenting UDP payload: {}:{} -> {}:{}, payload_len={}, udp_len={}, frag_size={}",
            src_ip,
            src_port,
            dst_ip,
            dst_port,
        ...
        if udp_packet.len() <= frag_payload_size {
        ...
        for i in 0..num_frags {
            let offset = i * frag_payload_size;
            let end = (offset + frag_payload_size).min(total_len);
    ...
    /// Fragment and inject UDP payload as IPv6 fragments.
    pub fn inject_fragmented_udp_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
        ...
        let udp_header = build_udp_header(src_port, dst_port, payload);
        let mut udp_packet = [&udp_header[..], payload].concat();
        if udp_packet.len() <= frag_payload_size {
            return self.inject_single_udp_v6(src_ip, dst_ip, src_port, dst_port, payload);
        }

        info!(
            "Fragmenting UDP payload (IPv6): [{}]:{} -> [{}]:{}, payload_len={}, udp_len={}, frag_size={}",
            src_ip,
            src_port,
            dst_ip,
            dst_port,
        ...
        let udp_checksum = calculate_udp_checksum_v6(src_ip, dst_ip, &udp_packet);
        udp_packet[6] = (udp_checksum >> 8) as u8;
        udp_packet[7] = (udp_checksum & 0xFF) as u8;

        let total_len = udp_packet.len();
        let identification = next_ipv6_fragment_id();
        let mut frags_sent = 0usize;
        let mut offset = 0usize;

        while offset < total_len {
            let remaining = total_len - offset;
            let is_last = remaining <= frag_payload_size;
            let frag_len = if is_last {
                remaining
            } else {
                frag_payload_size
            };
            let frag_data = &udp_packet[offset..offset + frag_len];
            ...
            let packet = [&ipv6_header[..], &frag_header[..], frag_data].concat();
            ...
            if sent < 0 {
                let err = std::io::Error::last_os_error();
                return Err(anyhow!(
                    "Failed to send IPv6 UDP fragment: {} (os error {})",
                    err,
                    err.raw_os_error().unwrap_or(-1)
                ));
            }
    ...
    /// Inject a single IPv6 UDP packet without fragmentation.
    fn inject_single_udp_v6(
        &self,
        src_ip: Ipv6Addr,
        dst_ip: Ipv6Addr,
        src_port: u16,
        ...
        if sent < 0 {
            let err = std::io::Error::last_os_error();
            return Err(anyhow!(
                "Failed to send single IPv6 UDP packet: {} (os error {})",
                err,
                err.raw_os_error().unwrap_or(-1)
            ));
        }

        debug!("Sent single IPv6 UDP packet: {} bytes", sent);
        Ok(1)
    }
}

/// Build UDP header
fn build_udp_header(src_port: u16, dst_port: u16, payload: &[u8]) -> Vec<u8> {
...
/// Build IPv6 fragment header (8 bytes)
fn build_ipv6_fragment_header(
    next_header: u8,
    offset_bytes: usize,
    more_fragments: bool,
    identification: u32,
    ...
    let mut offlg = offset_units << 3;
    if more_fragments {
        offlg |= 1;
    }
    header[2..4].copy_from_slice(&offlg.to_be_bytes());
...
/// Calculate TCP checksum for IPv6 with pseudo-header.
fn calculate_tcp_checksum_v6(src_ip: Ipv6Addr, dst_ip: Ipv6Addr, tcp_data: &[u8]) -> u16 {
    let mut pseudo_header = Vec::new();
    pseudo_header.extend_from_slice(&src_ip.octets());
    pseudo_header.extend_from_slice(&dst_ip.octets());
    pseudo_header.extend_from_slice(&(tcp_data.len() as u32).to_be_bytes());
    pseudo_header.extend_from_slice(&[0, 0, 0]); // zeros
    pseudo_header.push(6); // next header = TCP
    pseudo_header.extend_from_slice(tcp_data);
    calculate_checksum(&pseudo_header)
}

/// Calculate UDP checksum for IPv6 with pseudo-header.
fn calculate_udp_checksum_v6(src_ip: Ipv6Addr, dst_ip: Ipv6Addr, udp_data: &[u8]) -> u16 {
    let mut pseudo_header = Vec::new();
    pseudo_header.extend_from_slice(&src_ip.octets());
    pseudo_header.extend_from_slice(&dst_ip.octets());
    pseudo_header.extend_from_slice(&(udp_data.len() as u32).to_be_bytes());
    pseudo_header.extend_from_slice(&[0, 0, 0]); // zeros
    pseudo_header.push(libc::IPPROTO_UDP as u8); // next header = UDP
...
fn next_ipv6_fragment_id() -> u32 {
    IPV6_FRAGMENT_ID.fetch_add(1, Ordering::Relaxed)
}

fn next_ipv4_fragment_id() -> u16 {
...
mod tests {
    ...
    fn test_tcp_flags_for_split() {
        ...
        let second_flags_no_fin = if original_flags & 0x01 != 0 {
            0x18 | 0x01
        } else {
            0x18
        };
        ...
        let second_flags_with_fin = if original_flags_with_fin & 0x01 != 0 {
            0x18 | 0x01
        } else {
            0x18
        };
    ...
    fn test_tls_split_record_construction() {
        ...
        let original_payload: Vec<u8> = tls_header
            .iter()
        ...
        assert_eq!(
            &first_record[3..5],
            &(first_record_len as u16).to_be_bytes()
        );
        ...
        assert_eq!(
            &second_record[3..5],
            &(second_record_len as u16).to_be_bytes()
        );
    }
    ...
    fn test_disorder_payload_splitting() {
        ...
        let combined: Vec<u8> = first_part
            .iter()
    ...
    fn test_disorder_flags() {
        ...
        let second_flags_with_fin = if original_with_fin & 0x01 != 0 {
            0x18 | 0x01
        } else {
            0x18
        };
